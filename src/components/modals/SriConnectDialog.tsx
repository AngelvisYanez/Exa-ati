  ...
  return (
    ...
    >
      <div className="flex flex-col items-center text-center pb-4 pt-2">
        <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-brand-red to-brand-red-bright flex items-center justify-center text-white mb-4 shadow-lg shadow-brand-red/20 relative">
          <Building2 className="w-8 h-8" />
          <div className="absolute -bottom-2 -right-2 bg-success rounded-full p-1.5 border-2 border-white shadow-sm">
      ...
      {error && (
        <div className="bg-brand-red-subtle/80 border border-brand-red-pale rounded-xl p-3 text-[12.5px] text-brand-red font-bold mb-4 flex items-center gap-2 animate-in fade-in slide-in-from-top-2">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="bg-brand-gray-50 border border-brand-gray-100 rounded-xl p-3 flex gap-2 items-start mt-2">
          <Lock className="w-4 h-4 text-brand-gray-400 shrink-0 mt-0.5" />
          <p className="text-[10.5px] text-brand-gray-500 font-medium leading-relaxed">
            Tus credenciales están protegidas y cifradas de extremo a extremo con el estándar militar <strong className="text-brand-gray-700">AES-256</strong>. No se almacenan en texto plano.
          </p>
        </div>
        ...
        <div className="flex gap-2.5 mt-2">
          ...
          <Button
            type="submit"
            disabled={isLoadingState}
            className="flex-1 h-11 rounded-xl text-xs font-bold bg-brand-red hover:bg-brand-red-bright shadow-md shadow-brand-red/20 transition-all"
          >
            {isLoadingState ? (
