...
type ScrapeMode = 'comprobantes' | 'retenciones' | 'declaraciones' | 'obligaciones' | 'ats';

interface MassDownloadModalProps {
  open: boolean;
  onClose: () => void;
  /** Prefill fechas desde el periodo activo en Documentos. */
  initialDateRange?: { from: string; to: string };
  /** Dirección preferida según filtro de vista (emitidos/recibidos). */
  initialDirection?: Direction;
  /** Se llama cuando un job termina en COMPLETED (documentos ya sincronizados en BD). */
  onJobCompleted?: () => void;
}
...
const statusConfig: Record<JobStatus, { label: string; icon: typeof Loader2; color: string; bg: string }> = {
  ...
  PENDING: { label: 'Pendiente', icon: Clock, color: 'text-brand-red', bg: 'bg-brand-red-pale border-brand-red-light' },
};
...
const scrapeModeMeta: Record<ScrapeMode, { label: string; desc: string }> = {
  comprobantes: { label: 'Comprobantes', desc: 'Descarga XML/PDF del portal' },
  retenciones: { label: 'Retenciones', desc: 'Comprobantes tipo 6 (retención)' },
  declaraciones: { label: 'Declaraciones', desc: 'Consulta declaraciones presentadas' },
  obligaciones: { label: 'Obligaciones', desc: 'Perfil / vencimientos del contribuyente' },
  ats: { label: 'ATS', desc: 'Sync comprobantes + generación local' },
};
...
export default function MassDownloadModal({
  open,
  onClose,
  initialDateRange,
  initialDirection = 'ambos',
  onJobCompleted,
}: MassDownloadModalProps) {
  const [loading, setLoading] = useState(false);
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [direction, setDirection] = useState<Direction>(initialDirection);
  const [scrapeMode, setScrapeMode] = useState<ScrapeMode>('comprobantes');
  const logEndRef = useRef<HTMLDivElement>(null);
  const knownJobStatusRef = useRef<Map<string, string>>(new Map());
  const onJobCompletedRef = useRef(onJobCompleted);
  onJobCompletedRef.current = onJobCompleted;

  const defaultDesde = new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0];
  const defaultHasta = new Date().toISOString().split('T')[0];

  const [formData, setFormData] = useState({
    ruc: '',
    clave_sri: '',
    fecha_desde: defaultDesde,
    fecha_hasta: defaultHasta,
  ...
  useEffect(() => {
    ...
    if (initialDateRange?.from && initialDateRange?.to) {
      ...
      setFormData((prev) => ({
        ...prev,
        fecha_desde: initialDateRange.from,
        fecha_hasta: initialDateRange.to,
      }));
    }
  }, [open, initialDateRange?.from, initialDateRange?.to, initialDirection]);
  ...
  const fetchJobs = useCallback(async (manual = false) => {
    ...
    try {
      ...
      if (data.success && data.jobs) {
        const nextJobs = data.jobs as { id: string; status: string }[];
        const prev = knownJobStatusRef.current;
  ...
  const getActionType = (dir: Direction, mode: ScrapeMode): string => {
    if (mode === 'retenciones') return 'SCRAPE_RETENCIONES';
    if (mode === 'declaraciones') return 'SCRAPE_DECLARACIONES';
    if (mode === 'obligaciones') return 'SCRAPE_OBLIGACIONES';
    if (mode === 'ats') return 'SCRAPE_ATS';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('sri_access_token');
      const action = getActionType(direction, scrapeMode);
      const tipo =
        scrapeMode === 'retenciones'
          ? '6'
          : scrapeMode === 'ats'
            ? formData.tipo_comprobante || 'todos'
            : scrapeMode === 'comprobantes'
      ...
      if (response.ok && data.jobId) {
        toast.success('Trabajo encolado correctamente');
      } else {
  ...
  return (
    <Dialog open={open} onClose={onClose} size="2xl" showClose={false}>
      <div className="flex flex-col">
        {/* ─── Header ─── */}
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-brand-red to-brand-red-mid px-4 py-3 mb-5">
          ...
          <div className="relative flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-white/60">Lista y sincroniza comprobantes del portal al descargar</p>
              </div>
            </div>
            <button
              type="button"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-brand-gray-200">
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-3.5">
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border text-left transition-all duration-150 cursor-pointer ${
                            isActive
                              ? 'border-brand-red bg-brand-red-pale shadow-xs'
                              : 'border-brand-gray-200 bg-white hover:border-brand-gray-300'
                          }`}
                        >
                          <div className={`p-1 rounded-md transition-colors ${
                            isActive ? 'bg-brand-red text-white' : 'bg-brand-gray-100 text-brand-gray-400'
                          }`}>
                            <User className="h-3.5 w-3.5" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className={`text-sm font-medium truncate ${
                              isActive ? 'text-brand-red' : 'text-brand-gray-700'
                            }`}>
                              {e.ruc}
                            </div>
                            {e.razonSocial && (
              ...
              {hasCredentials ? (
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative group">
                    <div className="absolute left-3 top-1/2 -translate-y-1/2 text-brand-gray-400 group-focus-within:text-brand-red transition-colors duration-200">
                    ...
                    <Input
                      ...
                      autoComplete="off"
                      className="pl-9 h-9 text-sm border-brand-gray-200 bg-brand-gray-50/50 focus:bg-white focus:border-brand-red transition-all duration-200"
                      required
                    />
                  </div>
                </div>
              ...
              <div className="space-y-1.5">
                <Label className="text-[11px] font-semibold text-brand-gray-600">Acción</Label>
                <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">
                  {(Object.entries(scrapeModeMeta) as [ScrapeMode, typeof scrapeModeMeta['comprobantes']][]).map(
                    ([key, meta]) => {
                      const isActive = scrapeMode === key;
                      return (
                        <button
                          key={key}
                          type="button"
                          onClick={() => setScrapeMode(key)}
                          className={`rounded-lg border px-2 py-2 text-left transition-all duration-200 cursor-pointer ${
                            isActive
                              ? 'border-brand-red bg-brand-red-pale'
                              : 'border-brand-gray-200 bg-white hover:border-brand-red/30'
                          }`}
                        >
                          <div className={`text-[11px] font-semibold ${isActive ? 'text-brand-red' : 'text-brand-gray-700'}`}>
                            {meta.label}
                          </div>
                          <div className="text-[10px] text-brand-gray-400 leading-tight">{meta.desc}</div>
                        </button>
                      );
                    }
                  )}
              ...
              {scrapeMode === 'comprobantes' || scrapeMode === 'ats' ? (
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="w-full h-9 px-3 rounded-lg border border-brand-gray-200 bg-brand-gray-50/50 text-sm focus:bg-white focus:border-brand-red focus:ring-2 focus:ring-brand-red/15 outline-none transition-all duration-200"
                  required
                >
                  {tipoOptions.map(o => (
                ...
                {dateMode === 'month' ? (
                  ...
                  >
                    <div className="space-y-1.5">
                      ...
                      <div className="relative group">
                        <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-brand-gray-400 group-focus-within:text-brand-red transition-colors duration-200 pointer-events-none" />
                        <Input
                          name="fecha_desde"
                          type="date"
                          value={formData.fecha_desde}
                          onChange={handleChange}
                          className="pl-9 h-9 text-xs border-brand-gray-200 bg-brand-gray-50/50 focus:bg-white focus:border-brand-red transition-all duration-200"
                          required
                        />
                      </div>
                    </div>
              ...
              <div className="space-y-1.5">
                    ...
                    return (
                      <button
                        ...
                        className={`relative flex flex-col items-center gap-1.5 px-2 py-2.5 rounded-xl border-2 transition-all duration-200 cursor-pointer ${
                          isActive
                            ? 'border-brand-red bg-brand-red-pale shadow-xs'
                            : 'border-brand-gray-200 bg-white hover:border-brand-red/30 hover:bg-brand-gray-50'
                        }`}
                      >
                        ...
                        {isActive && (
                          <motion.div
                            ...
                            className="absolute -top-1 -right-1 w-3 h-3 bg-brand-red rounded-full ring-2 ring-white"
                          />
                        )}
                      </button>
                    );
                ...
                <p className="text-[10px] text-brand-gray-400 leading-relaxed">
                  {scrapeModeMeta[scrapeMode].desc}
                </p>
              )}
              ...
              <Button
                type="submit"
                disabled={loading}
                className="relative w-full h-9 mt-2 overflow-hidden rounded-xl bg-brand-red text-white text-sm font-semibold hover:bg-brand-red-bright active:scale-[0.98] transition-all duration-150 cursor-pointer shadow-md shadow-brand-red/20"
              >
                {loading ? (
                  ...
                  <span className="flex items-center justify-center gap-2">
                    ...
                    {scrapeMode === 'comprobantes' ? 'Iniciar Descarga' : 'Encolar acción'}
                  </span>
                )}
              </Button>
            </form>
