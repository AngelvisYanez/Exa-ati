      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {result.warning === 'SRI_UNAVAILABLE' && (
            <div className="bg-brand-red-subtle border border-brand-red-pale rounded-lg p-3 text-xs text-brand-red">
          ...
          {errores.length > 0 && (
            <div className="max-h-32 overflow-y-auto text-xs space-y-1 bg-brand-red-subtle border border-red-100 rounded-lg p-2">
              ...
              {errores.slice(0, 8).map((d) => (
                <p key={d.claveAcceso} className="text-brand-red font-mono">
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-full bg-brand-red text-white py-2 rounded-lg text-xs font-bold hover:bg-brand-red-bright transition-colors"
          >
            Cerrar
          </button>
        </div>
