...
const estadoConfig: Record<string, { label: string; color: string; bg: string }> = {
  DESCONECTADO: { label: "Desconectado", color: "text-brand-gray-600", bg: "bg-brand-gray-100" },
  VINCULANDO: { label: "Vinculando…", color: "text-amber-700", bg: "bg-amber-50" },
  CONECTADO: { label: "Conectado", color: "text-success", bg: "bg-success-pale" },
};
...
export default function WhatsAppMobilePanel() {
  ...
  const handleConfirmWaScan = async () => {
    try {
      ...
      if (res.success) {
        setWaEstado(res.estado || "CONECTADO");
        setWaNumero(res.numero || waNumero);
        setActionMsg(
          res.estado === "PENDIENTE"
            ? "WhatsApp en espera de confirmación del bridge."
  ...
  return (
    <div className="flex flex-col gap-5">
      {actionMsg && (
        <div className="bg-sky-50 border border-sky-200 text-brand-sky text-sm rounded-xl px-4 py-3">
          {actionMsg}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-5">
        <div className="flex flex-col gap-5">
          ...
          <section className="bg-white border border-brand-gray-200 rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-brand-gray-100 flex items-center justify-between gap-3">
              <div>
                <h3 className="text-[13px] font-bold text-brand-gray-700 uppercase tracking-wide">
                  Acceso móvil
                </h3>
              ...
              <span className="text-[10px] font-semibold text-brand-gray-400 uppercase shrink-0">
                LAN · {localIp || "localhost"}
              </span>
            </div>
            <div className="p-5 flex flex-col sm:flex-row gap-5 items-center">
              <div className="flex-1 text-sm text-brand-gray-600 space-y-2">
                <p>
                  Ambos dispositivos deben estar en la misma red Wi-Fi. El enlace expira en 30 días.
                </p>
                {mobileUrl && (
                  <p className="text-[11px] font-mono text-brand-gray-400 bg-brand-gray-50 border border-brand-gray-100 rounded-lg px-3 py-2 break-all">
                    {mobileUrl}
                  </p>
                )}
              </div>
              <div className="shrink-0 bg-brand-gray-50 border border-brand-gray-200 rounded-xl p-3">
                {loadingMobile ? (
                  <div className="w-40 h-40 flex items-center justify-center text-xs text-brand-gray-400">
                    Generando QR…
                  </div>
                ) : mobileQr ? (
                  <img src={mobileQr} alt="QR acceso móvil" className="w-40 h-40 block" />
                ) : (
                  <div className="w-40 h-40 flex items-center justify-center text-xs text-brand-gray-400 text-center px-3">
          ...
          <section className="bg-white border border-brand-gray-200 rounded-xl overflow-hidden">
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-[1fr_180px] gap-5">
              <div className="flex flex-col gap-4">
                {waEstado === "DESCONECTADO" && (
                  <div className="flex flex-col gap-2">
                    <label className="text-[11px] font-semibold text-brand-gray-500 uppercase">
                      Número (con código de país)
                    </label>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        className="flex-1 border border-brand-gray-200 rounded-lg px-3 py-2 text-sm font-mono text-brand-gray-800 outline-none focus:border-brand-red focus:ring-1 focus:ring-brand-red/20"
                      ...
                      <button
                        ...
                        disabled={loadingWa}
                        className="bg-brand-red hover:bg-brand-red-bright text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors disabled:opacity-50 cursor-pointer shrink-0"
                      >
                        Generar QR
                      </button>
                    </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <button
                        ...
                        disabled={loadingWa}
                        className="bg-success hover:bg-success text-white text-sm font-semibold px-4 py-2 rounded-lg cursor-pointer disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        className="border border-brand-gray-200 text-brand-gray-600 text-sm font-semibold px-4 py-2 rounded-lg hover:bg-brand-gray-50 cursor-pointer"
                      >
                        Cancelar
                      </button>
                    </div>
                ...
                {waEstado === "CONECTADO" && (
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center justify-between bg-success-pale border border-emerald-100 rounded-xl px-4 py-3">
                      <div>
                        <p className="text-[10px] text-success font-semibold uppercase">Número vinculado</p>
                        <p className="text-sm font-bold text-success font-mono mt-0.5">{waNumero}</p>
                      </div>
                    </div>

                    <div className="flex flex-col gap-2 border-t border-brand-gray-100 pt-3">
                      <p className="text-[11px] font-semibold text-brand-gray-500 uppercase">Preferencias WhatsApp</p>
                      <label className="flex items-start gap-2.5 text-sm text-brand-gray-700 cursor-pointer">
                        <input
                          type="checkbox"
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <button
                        ...
                        className="flex-1 min-w-[140px] border border-brand-gray-200 text-brand-gray-700 text-sm font-semibold px-4 py-2 rounded-lg hover:bg-brand-gray-50 cursor-pointer"
                      >
                        Enviar prueba
                      </button>
                      <button
                        ...
                        disabled={loadingWa}
                        className="border border-brand-red-pale text-brand-red text-sm font-semibold px-4 py-2 rounded-lg hover:bg-brand-red-subtle cursor-pointer disabled:opacity-50"
                      >
                        Desvincular
                      </button>
                    </div>
              ...
              <div className="flex items-center justify-center bg-brand-gray-50 border border-brand-gray-200 rounded-xl p-3 shrink-0">
                {loadingWa ? (
                  <div className="w-36 h-36 flex items-center justify-center text-xs text-brand-gray-400">Procesando…</div>
                ) : waEstado === "VINCULANDO" && waQr ? (
                  <img src={waQr} alt="QR WhatsApp" className="w-36 h-36 block" />
                ) : waEstado === "CONECTADO" ? (
                  <div className="w-36 h-36 flex flex-col items-center justify-center text-success text-center">
                    <span className="text-3xl mb-1">✓</span>
                    <span className="text-xs font-bold">Activo</span>
                  </div>
                ) : (
                  <div className="w-36 h-36 flex flex-col items-center justify-center text-brand-gray-300 text-center text-xs px-3">
                    <MessageSquare className="w-6 h-6 text-brand-gray-400 mb-1" />
                    QR de vinculación
                  </div>
                )}
              </div>
        ...
        {/* Vista previa chat */}
        <section className="bg-white border border-brand-gray-200 rounded-xl overflow-hidden flex flex-col h-fit xl:sticky xl:top-6">
          <div className="px-4 py-3 border-b border-brand-gray-100 bg-[#075e54] text-white">
          ...
          >
            {waMessages.length === 0 && !waTyping && (
              <div className="self-center text-center text-[11px] text-brand-gray-500 px-4 py-8">
              ...
              return (
                <div
                  ...
                  className={`max-w-[90%] px-2.5 py-1.5 rounded-lg text-xs leading-relaxed shadow-sm ${
                    isIn
                      ? "self-start bg-white rounded-tl-none text-brand-gray-800"
                      : "self-end bg-[#dcf8c6] rounded-tr-none text-brand-gray-800"
                  }`}
                >
                  {msg.html ? (
                    <div
                  ...
                  <span className="text-[9px] text-brand-gray-400 float-right ml-2 mt-1">{msg.time}</span>
                </div>
              );
            })}
            {waTyping && (
              <div className="self-start bg-white rounded-lg px-3 py-2 text-xs text-brand-gray-400">
                escribiendo…
              </div>
            )}
              ...
              <div className="flex flex-col gap-1.5 mt-1">
                {["¿Cuánto debo pagar de IVA?", "¿Cuáles son mis obligaciones?"].map((q) => (
                  <button
                    ...
                    className="bg-white hover:bg-brand-gray-50 text-[#075e54] text-[11px] font-semibold py-2 rounded-lg border border-brand-gray-200 cursor-pointer"
                  >
                    {q}
                  </button>
                ))}
          ...
          <div className="p-2 border-t border-brand-gray-100 flex gap-2 bg-white">
            <input
              className="flex-1 border border-brand-gray-200 rounded-full px-3 py-2 text-xs outline-none focus:border-brand-red"
