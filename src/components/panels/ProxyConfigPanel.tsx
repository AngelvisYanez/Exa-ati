    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-4">
        <div className="bg-brand-gray-50 border border-brand-gray-200 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-brand-red mt-1">{stats.total}</p>
        </div>
        <div className="bg-success-pale border border-success-light/40 rounded-lg p-4">
          <p className="text-xs font-semibold text-success uppercase tracking-wide">Disponibles</p>
        ...
        <div className={`border rounded-lg p-4 ${testingAll ? "bg-sky-50 border-sky-200" : "bg-brand-gray-50 border-brand-gray-200"}`}>
          <p className="text-xs font-semibold text-brand-gray-500 uppercase tracking-wide">Salud</p>
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-xl font-bold text-success">{healthCounts.alive}</span>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <Table className="w-full text-left text-sm border-collapse">
              <TableHeader>
                <TableRow className="border-b border-brand-gray-100 bg-brand-gray-50/50">
                  <TableHead className="px-5 py-3 font-semibold text-brand-gray-600 text-xs uppercase tracking-wide">Proxy</TableHead>
                  <TableHead className="px-5 py-3 font-semibold text-brand-gray-600 text-xs uppercase tracking-wide">País</TableHead>
                  <TableHead className="px-5 py-3 font-semibold text-brand-gray-600 text-xs uppercase tracking-wide">Estado</TableHead>
                  <TableHead className="px-5 py-3 font-semibold text-brand-gray-600 text-xs uppercase tracking-wide">Salud</TableHead>
                  <TableHead className="px-5 py-3 font-semibold text-brand-gray-600 text-xs uppercase tracking-wide">Latencia</TableHead>
                  <TableHead className="px-5 py-3 font-semibold text-brand-gray-600 text-xs uppercase tracking-wide">Uso</TableHead>
                  <TableHead className="px-5 py-3 font-semibold text-brand-gray-600 text-xs uppercase tracking-wide text-right">Acciones</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody className="divide-y divide-brand-gray-100">
                {proxies.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="p-0">
                      <EmptyState
                        ...
                        title="No hay proxies registrados."
                        compact
                      />
                    </TableCell>
                  </TableRow>
                  ...
                  sortedProxies.map(p => {
                    ...
                    return (
                      <TableRow key={p.id} className={`hover:bg-brand-gray-50/50 transition-colors ${testResult && !testResult.alive ? "opacity-60" : ""}`}>
                        ...
                        <TableCell className="px-5 py-3 font-medium text-brand-gray-600">{p.pais}</TableCell>
                        <TableCell className="px-5 py-3">
                          <button
                            onClick={() => handleToggleProxy(p.id, Boolean(p.activo))}
                            className={`relative w-9 h-5 rounded-full transition-colors shrink-0 cursor-pointer ${p.activo ? "bg-success" : "bg-brand-gray-200"}`}
                          >
                            <div
                        ...
                        <TableCell className="px-5 py-3">
                          {isTesting ? (
                            <Badge variant="outline" className="bg-sky-50 text-brand-sky border-sky-200 text-[10px] animate-pulse">
                              Probando...
                            </Badge>
                          ) : !testResult ? (
                            ...
                            <Badge variant="outline" className="bg-success-pale text-success border-success-light/40 text-[10px]">
                              Vivo
                            </Badge>
                          ) : (
                            <Badge variant="outline" className="bg-brand-red-subtle text-brand-red border-brand-red-pale text-[10px]">
                              Caído
                            </Badge>
                          )}
                        </TableCell>
                        <TableCell className="px-5 py-3">
                          {testResult && testResult.alive ? (
                            <span className="text-xs font-mono text-success">{testResult.latency} ms</span>
                          ) : testResult && !testResult.alive ? (
                        ...
                        <TableCell className="px-5 py-3 text-right">
                          <span className="inline-flex gap-1">
                            <Button
                              ...
                              disabled={isTesting}
                              className="text-brand-gray-500 hover:text-brand-red hover:bg-brand-gray-100 h-8 w-8"
                              title="Probar proxy"
                            >
                            ...
                            <Button
                              ...
                              className="text-red-500 hover:text-brand-red hover:bg-brand-red-subtle h-8 w-8"
