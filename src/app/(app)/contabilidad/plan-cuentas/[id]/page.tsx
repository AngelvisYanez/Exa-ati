  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const cuenta = cuentaData.cuenta || cuentaData.data || cuentaData;
        setCodigo(cuenta.codigo || "");
        setNombre(cuenta.nombre || "");
        setNivel(cuenta.nivel || 1);
        setTipo(cuenta.tipo || "ACTIVO");
        setEsAuxiliar(cuenta.esAuxiliar || false);
        setPermiteMovimiento(cuenta.permiteMovimiento ?? true);
        setCuentaPadreId(cuenta.cuentaPadreId || "");

        const padreData = await padreRes.json();
        setCuentasPadre(padreData.cuentas || padreData.data || []);
      } catch {
  ...
  if (loading) {
    return (
      <>
        ...
        <main className="p-3 flex-1 flex items-center justify-center text-sm text-brand-gray-500 animate-pulse">Cargando cuenta...</main>
      </>
    );
  }
        ...
        <Card className="p-5 border-brand-gray-200">
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select value={cuentaPadreId} onChange={(e) => setCuentaPadreId(e.target.value)} className="h-8 rounded-lg border border-input bg-transparent px-2.5 text-sm focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 outline-none">
                <option value="">Sin cuenta padre (raíz)</option>
                {cuentasPadre.filter((c) => c.id !== id).map((c) => (
