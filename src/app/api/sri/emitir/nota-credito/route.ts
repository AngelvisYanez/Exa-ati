  ...
  try {
    ...
    const notaCreditoXmlData = {
      ...
      infoNotaCredito: {
        ...
        codDocModificado: dto.docModificado.tipo,
        numDocModificado: dto.docModificado.numero,
        fechaEmisionDocSustento: dto.docModificado.fecha,
    ...
    const xmlSinFirma = xmlBuilder.buildNotaCredito(notaCreditoXmlData);
    ...
    await db.query(
      `INSERT INTO comprobantes (
        ...
        doc_modificado_tipo, doc_modificado_numero, doc_modificado_fecha, motivo, valor_modificacion
      ) VALUES (gen_random_uuid(), ?, '04', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        ...
        dto.docModificado.tipo, dto.docModificado.numero,
        dto.docModificado.fecha.includes('/') ? dto.docModificado.fecha.split('/').reverse().join('-') : dto.docModificado.fecha,
        dto.motivo, totales.importeTotal
      ]
    );
    ...
    if (emisor.tenant_id) {
      try {
        ...
        console.warn('[Emitir NC] Post-sync falló:', syncErr.message);
      }
    }
    ...
    console.error('[Emit Nota Credito Error]', error);
    return NextResponse.json(
      { message: error.message || 'Error interno del servidor al emitir nota de crédito' },
