import { realisticClick } from './sri-utils';
// @ts-ignore
...
export async function ensureSession(
  ...
  if (needsLogin) {
    ...
    if (isHeadless) {
      ...
      if (!loggedIn) {
         throw new Error(
           "El login automático falló o requiere resolver un CAPTCHA manualmente, lo cual no es soportado en Vercel Serverless. " +
           "Por favor, revisa tus credenciales o el saldo de AntiCaptcha."
         );
      }
    } else {
