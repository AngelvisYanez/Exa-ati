...
export const sriSoapClient = {
  ...
  async autorizarComprobante(
    ...
    const timeoutMs = options?.timeoutMs ?? SOAP_TIMEOUT_MS;
    const maxRetries = options?.maxRetries ?? SOAP_MAX_RETRIES;
  ...
  /** Consulta rápida (1 intento) para recuperar XML al generar RIDE. */
  async autorizarComprobanteRapido(claveAcceso: string): Promise<any> {
    return this.autorizarComprobante(claveAcceso, { timeoutMs: 12000, maxRetries: 1 });
  },
