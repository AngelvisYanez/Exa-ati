...
export function generarClaveAcceso(data: any): string {
  const claveData: ClaveAccesoData = {
    fechaEmision: data.fechaEmision || new Date(),
    tipoComprobante: data.tipoComprobante || '01',
    ruc: data.ruc,
    ambiente: data.ambiente || Ambiente.PRUEBAS,
    establecimiento: data.establecimiento || data.serie?.substring(0, 3) || '001',
    puntoEmision: data.puntoEmision || data.serie?.substring(3, 6) || '001',
    secuencial: data.secuencial || '000000001',
    codigoNumerico: data.codigoNumerico,
    tipoEmision: data.tipoEmision || TipoEmision.NORMAL,
    establecimientoTemporal: data.establecimientoTemporal
  };
  return claveAccesoService.generate(claveData);
}
