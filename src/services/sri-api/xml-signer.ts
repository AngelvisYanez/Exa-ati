...
export const xmlSigner = {
  async loadEmisorCertificate(
    ...
    const encryptedPassword = pickEncryptedCertPassword(emisor);
    if (encryptedPassword) {
      try {
        password = await encryption.decrypt(encryptedPassword);
      } catch {
        console.warn(`[Signer] No se pudo descifrar password de certificado para ${ruc}, usando valor directo`);
