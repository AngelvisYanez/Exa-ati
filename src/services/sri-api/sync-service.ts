...
async function procesarComprobanteSri(
  ...
  const respuestaSri = opts?.rapido
    ? await sriSoapClient.autorizarComprobanteRapido(comp.clave_acceso)
    : await sriSoapClient.autorizarComprobante(comp.clave_acceso);
...
async function importarClaveDesdeSri(
  claveAcceso: string,
  tenantId: string,
  userRuc: string,
  opts?: { rapido?: boolean }
  ...
  const respuestaSri = opts?.rapido
    ? await sriSoapClient.autorizarComprobanteRapido(claveAcceso)
    : await sriSoapClient.autorizarComprobante(claveAcceso);
  if (!respuestaSri.autorizaciones?.autorizacion) {
    return {
      claveAcceso,
...
export async function sincronizarConSri(
  ...
  const clavesSolicitadas = (options.clavesAcceso || []).filter(
    (c) => typeof c === 'string' && c.length === 49
  );

  // Si se piden claves concretas, procesar solo esas (evita que limite:1 omita la clave)
  if (clavesSolicitadas.length > 0 && !periodoActivo) {
    ...
    for (let i = 0; i < clavesSolicitadas.length; i++) {
      ...
      try {
        const existing = await db.queryOne<any>(
          `SELECT id, clave_acceso, secuencial, tipo, estado, fecha_emision, emisor_ruc
           FROM comprobantes WHERE clave_acceso = ?`,
          [claveAcceso]
        );

        if (existing) {
          const result = await procesarComprobanteSri(existing, tenantId, userRuc, { rapido: true });
          ...
          if (/Sin conexión al SRI|WSDL/i.test(result.detalle.accion)) {
            sriNoDisponible = true;
          }
        } else {
          const result = await importarClaveDesdeSri(claveAcceso, tenantId, userRuc, { rapido: true });
          detalle.push({
        ...
        if (/Sin conexión al SRI|WSDL|ETIMEDOUT|tiempo de espera/i.test(msg)) {
          sriNoDisponible = true;
        }
        errores++;
        detalle.push({
      ...
      if (i < clavesSolicitadas.length - 1 && !sriNoDisponible) await delay(DELAY_MS);
    }
    ...
    const result = {
      ...
      fechaHasta: null,
      truncado: false,
      warning: sriNoDisponible ? ('SRI_UNAVAILABLE' as const) : undefined,
    };
