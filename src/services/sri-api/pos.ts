...
import { saveAutorizadoXml } from './comprobante-importer';
...
export async function createPosInvoice(data: PosSaleData): Promise<{ claveAcceso: string; id: string }> {
  const emisor = await db.queryOne<any>(
    `SELECT id, ruc, razon_social, nombre_comercial, ambiente, tipo_emision,
            ...
            obligado_contabilidad, tenant_id,
            certificado_p12, password_certificado
  ...
  if (!emisor.certificado_p12 || !emisor.password_certificado) {
    throw new Error('El emisor no tiene certificado digital configurado. Súbelo en Configuración.');
  }
  ...
  if (result.estado === 'AUTORIZADO' && result.xmlAutorizado && comprobanteRecord?.id) {
  ...
  if (result.estado === 'AUTORIZADO' && emisor.tenant_id) {
    for (const item of data.items) {
      try {
        const producto = await db.queryOne<{ id: string; stock: string }>(
          `SELECT id, stock FROM productos
           WHERE tenant_id = $1 AND codigo = $2 AND activo = true`,
          [emisor.tenant_id, item.codigo]
        );
        ...
        await db
          ...
          .catch(() => {
            /* tabla opcional hasta migrar */
          });
      } catch (e) {
        console.warn('[POS] No se pudo descontar stock para', item.codigo, e);
      }
    }
  }
