...
export function buildXmlFactura(data: any): string {
  if (data.infoTributaria && data.infoFactura) {
    return xmlBuilder.buildFactura(data);
  }
  ...
  const structuredData = {
    infoTributaria: {
      ...
      razonSocial: data.razonSocial || 'EMPRESA PRUEBA S.A.',
      nombreComercial: data.nombreComercial,
      ruc: data.ruc || '0999000000001',
      claveAcceso: data.claveAcceso,
      codDoc: data.codDoc || '01',
      estab: data.estab || '001',
      ptoEmi: data.ptoEmi || '001',
      secuencial: data.secuencial || '000000001',
      dirMatriz: data.dirMatriz || 'Guayaquil, Ecuador'
    },
    infoFactura: {
      fechaEmision: data.fechaEmision || '31/07/2026',
      dirEstablecimiento: data.dirEstablecimiento || 'Guayaquil, Ecuador',
      obligadoContabilidad: data.obligadoContabilidad || 'NO',
      tipoIdentificacionComprador: data.tipoIdentificacionReceptor || data.tipoIdentificacionComprador || '04',
      razonSocialComprador: data.razonSocialReceptor || data.razonSocialComprador || 'CLIENTE PRUEBA S.A.',
      identificacionComprador: data.identificacionReceptor || data.identificacionComprador || '0999000000002',
      totalSinImpuestos: data.totalSinImpuestos || 100.00,
      totalDescuento: data.totalDescuento || 0.00,
      ...
      importeTotal: data.importeTotal || 115.00,
      moneda: data.moneda || 'USD',
      pagos: data.pagos || [{ formaPago: '01', total: data.importeTotal || 115.00 }]
    },
    detalles: data.detalles || []
  };

  return xmlBuilder.buildFactura(structuredData);
}
