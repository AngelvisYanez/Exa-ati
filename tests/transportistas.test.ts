...
describe('transportistas', () => {
  ...
  it('getByTenant filtra por tenant', async () => {
    ...
    const { getByTenant } = await import('../src/services/sri-api/transportistas');
  ...
  it('getById retorna transportista', async () => {
    ...
    const { getById } = await import('../src/services/sri-api/transportistas');
  ...
  it('create valida RUC de 13 dígitos terminado en 001', async () => {
    const { create } = await import('../src/services/sri-api/transportistas');
  ...
  it('update modifica campos', async () => {
    mockDb.queryOne.mockResolvedValue(TRANSPORTISTA);
    const { update } = await import('../src/services/sri-api/transportistas');
  ...
  it('remove desactiva transportista', async () => {
    mockDb.queryOne.mockResolvedValue(TRANSPORTISTA);
    const { remove } = await import('../src/services/sri-api/transportistas');
    await remove('uuid-1');
  ...
  it('hardDelete elimina físicamente', async () => {
    mockDb.queryOne.mockResolvedValue(TRANSPORTISTA);
    const { hardDelete } = await import('../src/services/sri-api/transportistas');
